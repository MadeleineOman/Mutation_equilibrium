...
intron_total_length = int(length_DNA)
...
DNA = ""
for pos in range(0,intron_total_length): 
    DNA += choice(["A", "T", "C", "G"])
DNA_map = "N"*len(DNA)
print("all intron, all randomly generated seq")
...
text_file = open("Scripting_data_trial3_simple/{t}_DNA_gen0_cds.txt".format(t=NameForThisSim), "w")                                               #! need to change file creation: use the {}.format... file structure? 
...
for key in model.keys(): 
    mutation_dictionary[key] = 0

#making it so only muts in sections of 10 
for sb in tqdm(range(int(len(DNA)/10))): 
    section_dna = DNA[sb:sb+10]
    section_weights = weights[sb:sb+8]
    cb = base_to_mutate(section_dna, section_weights)-1 # so it choosed 0:7 not 1:8 
    c_triplet = section_dna[cb:cb+3]
    
    mutation_dictionary[c_triplet] += 1
    
    
    curr_into_triplets = list(key for key in model[c_triplet][1].keys())
    current_weights_to_turn_into = list(value for value in model[c_triplet][1].values()) # ex. "ATCGTA" --> [0.3, 0.5, 0.1, 0.1] (weights of triplet base changes) (these are already normalized!)
    

    mb =  choice(curr_into_triplets, p = current_weights_to_turn_into)[1]

    DNA = DNA[:sb+cb+1]+mb+DNA[sb+cb+2:]
    
file = open("Scripting_data_trial3_simple/{t}_DNA_final_cds_invariant50.txt".format(t=NameForThisSim), "w+")
file.write(DNA)
file.close()
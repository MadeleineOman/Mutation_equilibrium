...
# Description_of_sim,  Number_gens, len_DNA 
#NameForThisSim = sys.argv[1]  --> where "name of this sim" is the prefix of the files in the directory specified below
#number_gens = int(sys.argv[2])
#length_DNA = int(sys.argv[3])

# only using exons with >4000bp 
...
text_file = open("data/{t}_DNA_gen0_cds.txt".format(t=NameForThisSim), "w")  #! need to change file creation: use the {}.format... file structure? 
n = text_file.write(DNA)
text_file.close()
text_file = open("data/{t}_DNA_map_cds_invariant50.txt".format(t=NameForThisSim), "w")
n = text_file.write(DNA_map)
text_file.close()
text_file = open("data/{t}_exon_insertion_dict.txt".format(t=NameForThisSim), "w")
n = text_file.write(json.dumps(exons_insertion_position_dict))
text_file.close()
text_file = open("data/{t}_exons_toUseSeqeunce_dict.txt".format(t=NameForThisSim), "w")
...
#         NameForThisSim =(sys.argv[1]) 
#         file = open("data/dif_DNA/{t}_DNA_final_cds_invariant50_{g}.txt".format(t=NameForThisSim, g=x), "w+")
#         file.write(DNA)
#         file.close()
...
file = open("data/{t}_avMutability_cds_invariant50.txt".format(t=NameForThisSim), "w+")
file.write(str(av_mutability))
file.close()

file = open("data/{t}_intronMut_cds_invariant50.txt".format(t=NameForThisSim), "w+")
file.write(str(intron_mut))
file.close()

file = open("data/{t}_exonMut_cds_invariant50.txt".format(t=NameForThisSim), "w+")
file.write(str(exon_mut))
file.close()

file = open("data/{t}_DNA_final_cds_invariant50_{g}.txt".format(t=NameForThisSim, g=x), "w+")
file.write(DNA)
file.close()
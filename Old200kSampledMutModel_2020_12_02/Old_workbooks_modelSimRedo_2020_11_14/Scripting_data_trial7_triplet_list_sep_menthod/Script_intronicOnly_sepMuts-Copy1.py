...
def base_to_mutate(DNA, current_weights, indices, mut_indices): 
    """
    ...
    while base_index in mut_indices:
        #print(base_index)
        base_index = choice(indices, p=normalized_weights) 
...
text_file = open("Scripting_data_trial7_sep/{t}_DNA_gen0_cds.txt".format(t=NameForThisSim), "w")                                               #! need to change file creation: use the {}.format... file structure? 
...
file = open("Scripting_data_trial7_sep/DNA_final_{t}.txt".format(t=NameForThisSim), "w+")
file.write(DNA)
file.close()

text_file = open("Scripting_data_trial7_sep/Triplet_mut_counts_{t}.txt".format(t=NameForThisSim), "w+")
text_file.write(json.dumps(mutation_dictionary))
text_file.close()
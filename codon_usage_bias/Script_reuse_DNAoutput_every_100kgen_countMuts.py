#about the script 
#1sr pos = name for the sim
#2nd the number of generations
#3rd the location of the dna to use 
#4th the location of the dna map to use

# using the updated genome wide model 
#blosum 90 
...
for x in tqdm(range(int(number_gens)+1)):
    ...
    if len(DNA)-ab_index >= 3 and ab_index >= 3 : #making sure dont pick the ends 
        ...
        if codingYesNo(ab_index, mb, DNA, DNA_map) == None:# non-coding region 
            ...
            c_triplet = DNA[ab_index-1: ab_index+2]
            c_triplet_left = DNA[ab_index-2: ab_index+1]
            c_triplet_right = DNA[ab_index: ab_index+3]
...
#         file.close()
        ...
        file = open("data/redo_multDif_exons_2021_02_07/{t}_DNA_final_cds_invariant50_{g}.txt".format(t=NameForThisSim, g = x), "w+")
        file.write(DNA)
        file.close()
...
text_file = open("data/redo_multDif_exons_2021_02_07/{t}_triplet_into_count_dict.txt".format(t=NameForThisSim), "w")
n = text_file.write(json.dumps(triplet_into_count_dict))
text_file.close()

text_file = open("data/redo_multDif_exons_2021_02_07/{t}_triplet_chosen_count_dict.txt".format(t=NameForThisSim), "w")
